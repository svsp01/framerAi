...
"use client"
import React, { useState, useRef, useEffect } from 'react';
import { Sparkles, ThumbsUp, ThumbsDown, Heart, ShoppingCart, AlertCircle } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { Button } from "@/components/ui/button";
import { Textarea } from "@/components/ui/textarea";
import { ScrollArea } from "@/components/ui/scroll-area";
...
const Chat = () => {
    ...
    const handleSend = async () => {
        if (input.trim()) {
            ...
            try {
                ...
                setMessages((prev: any) => [...prev, { type: 'error', content: 'Failed to generate image.' }]);
            } finally {
                setIsGenerating(false);
            }
        }
    ...
    const handleAction = (index: number, action: string) => {
        setMessages((prev: any) => prev.map((msg: any, i: number) =>
            i === index && msg.type === 'image'
                ? { ...msg, actions: { ...msg.actions, [action]: !msg.actions[action] } }
                : msg
        ));
